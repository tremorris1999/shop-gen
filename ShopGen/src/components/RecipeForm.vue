  ...
  <v-card width="378px" class="mx-auto">
    ...
    <v-card-text style="max-height: 524px">
      <v-form>
        ...
        <template v-for="i in currentRecipe.ingredients" :key="i.id">
          <v-row no-gutters>
            <v-col cols="4">
              <v-text-field
                class="field"
        ...
        <v-row
          v-for="(imageId, index) in currentImageIds"
          :key="index"
          justify="start"
        >
          <ImageThumbnail
    ...
    <v-card-actions>
      <v-btn color="primary" variant="tonal" @click="save" :loading="isSaving"
        >Save</v-btn
      >
      <v-btn
        color="secondary"
...
onBeforeMount(async () => {
  if (props.recipe)
    try {
      currentRecipe.value.ingredients = await api.getIngredients(
        props.recipe?.id
      )
    } catch {
...
const addIngredient = () => {
  currentRecipe.value.ingredients.splice(
    currentRecipe.value.ingredients.length,
    0,
...
const removeIngredient = (ingredient: Ingredient) => {
  ...
  if (index > -1) currentRecipe.value.ingredients.splice(index, 1)
}
...
const queueImageDelete = (id: string) => {
  ...
  if (index > -1) currentImageIds.value.splice(index, 1)
}
